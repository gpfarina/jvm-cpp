...
int main(int argc, char** argv) {
  ...
  if (_classfile.getMajorVersion() != 65) {
    std::cerr << "FAIL: Expected major version 65, got "
              << _classfile.getMajorVersion() << std::endl;
    passed = false;
  }
  if (_classfile.getConstantPoolCount() != 15) {
    std::cerr << "FAIL: Expected constant pool count 15, got "
              << _classfile.getConstantPoolCount() << std::endl;
    passed = false;
  }
